...
int Menu::load() {
    ...
    getimage(ExpImg, GLO_PNG, 2 * GRID, 59 * GRID, GRID, GRID);
...
int Menu::update() {
    ...
    putimage_withalpha(NULL, ExpImg, RrightBord, 72 + 3 * BIG_GRID);
    ...
    xyprintf(RrightBord + 5, 72 + 2 * BIG_GRID + 6, "%12d", health);
    xyprintf(RrightBord + 5, 72 + 3 * BIG_GRID + 6, "%12d", health);
    xyprintf(RrightBord + 5, 72 + 4 * BIG_GRID + 6, "%12d", health);
    return 0;
}
